...
aggro_CD8 <- AggregateExpression(gex.CD8, return.seurat = T, group.by = "Celltype")
  ...
  #Filter to take only the top enriched genes and remove unnecessary genes.
...
aggro_CD8 <- AddModuleScore_UCell(aggro_CD8, features = gene_list, name = "")



mat<- aggro_CD8@meta.data[,names(gene_list)] %>% as.matrix()
...
pdf("Fig/CD8_Cluster_annotation_heatmap.pdf", height = 6, width = 7)
